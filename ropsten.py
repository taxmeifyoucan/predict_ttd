...
granuality=300 #Step in seconds
...
def construct_polynom():
    ...
    if ttime:
        return(estimate_time(1654696800, predict, predict_time, predictp, predictm))
...
def estimate_hashrate(data):
    ...
    target=50000000000000000
    time_now=web3.eth.get_block('latest')['timestamp']
    time_target=1654696800
    ...
    print("Current hashrate is %.2f GH/s <p></p>" % hashrate)
    hash_target=((target-current_ttd)/(time_target-time_now)/1000000000)
    
    print("To achieve TTD", target, "at", time.ctime(time_target),"UTC, around %.2f GH/s in network is needed <p></p>" % hash_target)
...
def draw_chart(target_t, target_y, y, t, predict_err, predictm):
   ...
   # plt.plot(dates,h(t), color='purple', linestyle='dashed')
    ...
    plt.scatter(ds, y) 
    plt.plot(ds, y) 
    plt.plot(target_t,500000000000,'ro', color='green') 
...
def estimate_ttd(target, polynom_ttd, polynom_time, predict_err, predict_m):
 ...
 #       print("TTD of", int(args['ttd']), "was achieved at block", block_by_ttd(target*100000, 1, latest_block),"<p></p>")
        ...
        print("Current TTD is", currenttd,"and using latest data linearly, around", datetime.timedelta(seconds =int(timeleft)), "is left to achieve the target.")


#Returns estimated TTD value at given timestamp
def estimate_time(target, polynom_ttd, polynom_time, predict_err, predict_m):
    
    point=target

    draw_chart(1654696800, int(np.polyval(polynom_ttd,point)*100000),y,t,predict_err, predict_m)
