...
granuality=14400 #Step between data points
...
def draw_chart(target_t, target_y, poly_h, poly_l):
    ...
    plt.plot(target_t,target_y/1000000000000000000000000000,'ro', color='green') 
...
def update(blockn, step):
    ...
    while blockn < latest_block:
        ttd=int(web3.eth.getBlock(blockn).totalDifficulty / 1000000000000000000000000000)
...
def estimate_ttd(target):
    ...
    ttd_target=target/1000000000000000000000000000
    ...
    if current_ttd > target:
        ...
        timeleft=(int(target)-current_ttd)/(ttd_diff_avg*100000000000000000000000)*time_diff_avg
        if timeleft < 259200:
            print("Current total difficulty is", current_ttd,"and", dt.timedelta(seconds =int(timeleft)), "is left to achieve the target.")

        draw_chart(point,target,coeff_h, coeff_l)
        print("Terminal Total Difficulty of", int(target), "on <b>Gnosis chain</b> is expected around<b>", dt.datetime.utcfromtimestamp(point).strftime("%a %b %d %H:%M %Y"), "UTC</b>, i.e. between", dt.datetime.utcfromtimestamp(point_high).strftime("%a %b %d %H:%M %Y"),"UTC and", dt.datetime.utcfromtimestamp(point_low).strftime("%a %b %d %H:%M %Y"),"UTC")
...
def estimate_time(target):
    ...
    td=int(np.polyval(coeff_ttd, target)*100000000000000000000000)
