...
def estimate_hashrate(data):
    ...
    while i < l:
        i+=1
        d.append(int ((y[i]-y[i-1]) / (t[i]-t[i-1])))
    ...
    print("Current hashrate:", int(hashrate), "MH/s <p></p>")
...
def estimate_time(target, polynom_ttd, polynom_time, predict_err, predict_m):
    ...
    draw_chart(int(args['time']), int(np.polyval(polynom_ttd,point)*100000),y,t,predict_err, predict_m)
